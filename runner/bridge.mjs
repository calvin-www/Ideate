...
const controller = new RunnerController({
  createWorker: () => new Worker(new URL('./worker.mjs', import.meta.url), { type: 'module', credentials: 'omit' }),
  send: (message) => { if (parentOrigin) window.parent.postMessage(message, parentOrigin); },
});

window.addEventListener('message', (event) => {
  if (window.parent === window || !isTrustedParentEvent(event, window.parent, APP_ORIGINS)) return;
