  ...
  start() {
    ...
    try {
      ...
      this.timer = setTimeout(() => {
        ...
        this.failRuntime(
          "Python could not finish loading. Run again to retry.",
        );
      }, INIT_TIMEOUT_MS);
      ...
      worker.onerror = () => {
        if (this.worker === worker)
          this.failRuntime("Python runtime failed. Run again to retry.");
      };
    } catch {
  ...
  handle(message) {
    ...
    if (message.type === "init") {
      if (!this.worker) this.start();
        ...
        this.status(
          this.active?.startedAt !== null && this.active
            ? "running"
            : this.ready
              ? "ready"
              : "loading",
    ...
    if (message.type === "stop") {
      if (this.active?.request.id === message.id)
  ...
  execute() {
    ...
    this.timer = setTimeout(
      () =>
        this.finish(
          "timeout",
          "Execution stopped after the 10 second limit.",
          undefined,
          true,
        ),
      RUN_TIMEOUT_MS,
  ...
  receive(message) {
    if (!isRunnerMessage(message)) {
      if (this.active)
        this.finish(
          "error",
          "Python returned an invalid or oversized response.",
          undefined,
          true,
        );
      return;
    ...
    if (message.type === "status") {
      if (message.status === "ready" && !this.ready) {
        ...
        this.failRuntime(message.error || "Python failed to initialize.");
      return;
    }
    if (
      !this.active ||
    ...
    if (message.type === "output") {
      ...
      if (byteLength(message.text) > remaining)
        this.finish(
          "error",
          "Execution stopped: the combined output exceeded 64 KiB.",
          undefined,
          true,
        );
