...
export function createRunnerServer({
  appOrigins = ["http://localhost:3000", "http://127.0.0.1:3000"],
} = {}) {
  const origins = appOrigins.map((origin) => {
    const url = new URL(origin);
    if (!["http:", "https:"].includes(url.protocol) || url.origin !== origin)
      throw new Error("Runner app origins must be exact HTTP(S) origins.");
    return url.origin;
  });
  const csp = [
    "default-src 'none'",
  ...
  return createServer(async (request, response) => {
    ...
    let path;
    try {
      path = new URL(request.url, "http://runner.invalid").pathname;
    } catch {
    ...
    if (path === "/config.mjs") {
      ...
      response.end(
        request.method === "HEAD"
          ? undefined
          : `export const APP_ORIGINS = ${JSON.stringify(origins)};`,
      );
      return;
    }
...
if (
  ...
  const appOrigins = process.env.RUNNER_APP_ORIGINS?.split(",")
    .map((origin) => origin.trim())
    .filter(Boolean);
  const server = createRunnerServer({ appOrigins });
  server.listen(port, "127.0.0.1", () =>
    process.stdout.write(`Python runner ready at http://localhost:${port}\n`),
  );
  const shutdown = () => {
    server.close();
