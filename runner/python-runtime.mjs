import { byteLength, trimUtf8, MAX_CHUNK_BYTES, MAX_OUTPUT_BYTES } from './protocol.mjs';
  ...
  return {
    async execute(request, onOutput, onLimit = () => {}) {
      ...
      const flush = () => {
        ...
        if (pending) { outputEvents++; onOutput(pendingChannel, pending); }
        pending = '';
      };
      const append = (channel, text) => {
        ...
        if (pending && outputEvents < 128 && (outputEvents === 0 || pending.includes('\n'))) flush();
        else if (pending && !flushTimer) flushTimer = setTimeout(() => { flushTimer = undefined; flush(); }, 16);
      };
      const write = (channel) => (buffer) => {
        const allowed = Math.min(buffer.length, MAX_OUTPUT_BYTES - outputBytes);
        if (allowed > 0) {
          append(channel, decoders[channel].decode(buffer.subarray(0, allowed), { stream: true }));
          outputBytes += allowed;
        }
      ...
      pyodide.setStdin({ error: true });
      const globals = pyodide.runPython('{"__name__": "__main__", "__file__": "main.py"}');
      let result = { status: 'success' };
      try {
        pyodide.runPython(INPUT_SETUP, { globals });
        await pyodide.runPythonAsync(request.code, { globals, filename: 'main.py' });
      } catch (error) {
        ...
        result = { status: 'error', error: trimUtf8(message, MAX_CHUNK_BYTES), ...(line ? { line } : {}) };
      } finally {
        try { pyodide.runPython('import sys\nsys.stdout.flush()\nsys.stderr.flush()'); } catch { /* Preserve the program's original exception. */ }
        if (!overflow) {
      ...
      if (overflow) result = { status: 'error', error: 'Execution stopped: the combined output exceeded 64 KiB.' };
