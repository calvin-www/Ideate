...
beforeAll(async () => {
  ...
  server = await createServer({
    configFile: false, root: resolve('.'), cacheDir: cacheDirectory, logLevel: 'error',
    server: { host: '127.0.0.1', port: 0 },
    optimizeDeps: { include: ['@excalidraw/excalidraw'] },
    plugins: [{ name: 'adapter-test-page', configureServer(vite) {
      vite.middlewares.use((request, response, next) => {
        if (request.url !== '/') return next();
        response.setHeader('Content-Type', 'text/html');
        response.end('<!doctype html><html><body>Board adapter test</body></html>');
      });
    } }],
  });
  await server.listen();
  ...
  await page.evaluate("import('/src/features/board/adapter.ts').then(module => { window.buildBoardPatch = module.buildBoardPatch; window.normalizeBoardImport = module.normalizeBoardImport; window.sampleBoard = module.sampleBoard; })");
  await patch([], {});
}, 60_000);
afterAll(async () => {
  await browser?.close();
  await server?.close();
  if (cacheDirectory && dirname(resolve(cacheDirectory)) === resolve(tmpdir()) && basename(cacheDirectory).startsWith('ideate-board-tests-')) {
    await rm(cacheDirectory, { recursive: true, force: true });
  }
}, 30_000);

async function patch(elements: BoardElement[], changes: BoardPatch): Promise<BoardElement[]> {
  return page.evaluate(async ({ elements, changes }) => {
    const bridge = window as unknown as { buildBoardPatch: (elements: BoardElement[], changes: BoardPatch) => Promise<BoardElement[]> };
...
async function connectedScene() {
  const elements = await patch([], { additions: [shape, { ...shape, x: 300 }] });
  return patch(elements, { additions: [{ type: 'arrow', x: 125, y: 60, width: 170, height: 1, startId: elements[0].id, endId: elements[1].id }] });
}
...
describe('real Excalidraw board adapter', () => {
  ...
  it('rejects missing or unbindable arrow targets rather than silently dropping the binding', async () => {
    const [text] = await patch([], { additions: [{ type: 'text', x: 20, y: 20, width: 100, height: 30, text: 'label' }] });
    await expect(patch([text], { additions: [{ type: 'arrow', x: 0, y: 0, width: 100, height: 1, endId: 'missing' }] })).rejects.toThrow(/binding/i);
    const [arrow] = await patch([], { additions: [{ type: 'arrow', x: 0, y: 0, width: 100, height: 1 }] });
    await expect(patch([arrow], { additions: [{ type: 'arrow', x: 0, y: 10, width: 100, height: 1, endId: arrow.id }] })).rejects.toThrow(/binding/i);
  });

  it('resizes text from its real font metrics after an edit', async () => {
    const [text] = await patch([], { additions: [{ type: 'text', x: 20, y: 20, width: 100, height: 30, text: 'a' }] });
    const [updated] = await patch([text], { updates: [{ id: text.id, text: 'A considerably longer label\nwith a second line' }] });
    ...
    expect(updated.originalText).toBe('A considerably longer label\nwith a second line');
  ...
  it('keeps a resized bound label inside its container with the original label ID', async () => {
    ...
    const result = await patch(elements, { updates: [{ id: label.id, text: 'one\ntwo\nthree\nfour\nfive\nsix' }] });
    ...
    expect(Number(updated.y) + Number(updated.height)).toBeLessThanOrEqual(Number(container.y) + Number(container.height));
    expect(container.boundElements).toContainEqual({ id: label.id, type: 'text' });
  });
  ...
  it('removes reciprocal arrow bindings when a target is deleted', async () => {
    ...
    expect(noArrow.find((element) => element.id === elements[1].id)!.boundElements).toEqual([]);
  });

  it('deletes bound labels along with their container', async () => {
    const elements = await patch([], { additions: [{ ...shape, text: 'inside' }] });
    const result = await patch(elements, { deleteIds: [elements[0].id] });
  ...
  it('moves a container with its label and attached arrow endpoint', async () => {
    const withLabel = await patch([], { additions: [{ ...shape, text: 'inside' }, { ...shape, x: 300 }] });
    const elements = await patch(withLabel, { additions: [{ type: 'arrow', x: 125, y: 60, width: 170, height: 1, startId: withLabel[0].id, endId: withLabel[1].id }] });
    const oldLabel = elements.find((element) => element.type === 'text')!;
    const oldArrow = elements.find((element) => element.type === 'arrow')!;
    const result = await patch(elements, { updates: [{ id: withLabel[0].id, x: 70, y: 50 }] });
    ...
    expect(Number(arrow.x) + end[0]).toBeCloseTo(Number(oldArrow.x) + oldEnd[0]);
    expect(Number(arrow.y) + end[1]).toBeCloseTo(Number(oldArrow.y) + oldEnd[1]);
  ...
  it('does not mutate the source snapshot while creating reciprocal bindings', async () => {
    const result = await page.evaluate(async (shape) => {
      ...
      await bridge.buildBoardPatch(elements, { additions: [{ type: 'arrow', x: 125, y: 60, width: 100, height: 1, startId: elements[0].id }] });
  ...
  it('preserves earlier arrows when adding another connector to the same target', async () => {
    const elements = await connectedScene();
    const oldArrow = elements.find((element) => element.type === 'arrow')!;
    const result = await patch(elements, { additions: [{ type: 'arrow', x: 125, y: 70, width: 170, height: 1, startId: elements[0].id, endId: elements[1].id }] });
    expect(result.find((element) => element.id === oldArrow.id)!.startBinding).toEqual(oldArrow.startBinding);
    const bindings = result.find((element) => element.id === elements[0].id)!.boundElements as { id: string }[];
  ...
  it('preserves an unrelated element’s geometry and version when applying an update', async () => {
    const elements = await patch([], { additions: [shape, { ...shape, x: 300 }] });
    const result = await patch(elements, { updates: [{ id: elements[0].id, strokeColor: '#ff0000' }] });
  ...
  it('normalizes imported freehand strokes without losing their points or pressure', async () => {
    const drawing = { id: 'manual-stroke', type: 'freedraw', x: 10, y: 20, width: 40, height: 20, version: 7, versionNonce: 33, points: [[0, 0], [20, 20], [40, 5]], pressures: [0.2, 0.7, 0.3], simulatePressure: false, runtimeHandle: { private: true } };
    const result = await page.evaluate(async (drawing) => {
      const api = window as unknown as { normalizeBoardImport: (elements: BoardElement[]) => Promise<BoardElement[]> };
      return api.normalizeBoardImport([drawing]);
    }, drawing);
    expect(result).toHaveLength(1);
    expect(result[0]).toMatchObject({ id: drawing.id, type: 'freedraw', points: drawing.points, pressures: drawing.pressures, simulatePressure: false, version: 7 });
  ...
  it('normalizes imported array diagrams without changing IDs or their source snapshot', async () => {
    const result = await page.evaluate(async () => {
      const api = window as unknown as { normalizeBoardImport: (elements: BoardElement[]) => Promise<BoardElement[]>; sampleBoard: () => Promise<BoardElement[]> };
      ...
      return { beforeIds: elements.map(element => element.id), afterIds: normalized.map(element => element.id), labels: normalized.filter(element => element.type === 'text').map(element => element.text), untouched: before === JSON.stringify(elements) };
    });
    expect(result.afterIds).toEqual(result.beforeIds);
    expect(result.labels).toEqual(expect.arrayContaining(['2', '5', '8', '12', '16', '23', '38', '56']));
