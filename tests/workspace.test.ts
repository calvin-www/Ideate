...
describe("workspace edit boundaries", () => {
  it("retains recent history and linked sources while keeping a long session reloadable", () => {
    ...
    data.messages = Array.from({ length: 510 }, (_, i) => ({
      id: `m${i}`,
      role: i % 2 ? "assistant" : "user",
      text: `message ${i}`,
    }));
    data.runs = Array.from({ length: 105 }, (_, i) => ({
      id: `r${i}`,
      code: "print(42)",
      revision: 0,
      output: "42",
    ...
    data.references = Array.from({ length: 2010 }, (_, i) => ({
      id: `ref${i}`,
      tool: "code",
      revision: 0,
      label: "Python",
  ...
  it("does not apply a cancelled job", () => {
    expect(() => applyProposal(createWorkspace(), proposal(), null)).toThrow(
      /cancel/i,
    );
  });
  it("rejects stale source context even when the target has not changed", () => {
    ...
    expect(() =>
      applyProposal(
        data,
        { ...proposal(), sourceRevisions: { notes: 0 } },
        "job1",
      ),
    ).toThrow(/source/i);
  });
  ...
  it("preserves later manual edits when undo would conflict", () => {
    const after = editText(
      applyProposal(createWorkspace(), proposal(), "job1"),
      "code",
      "keep this",
    );
  ...
  it("rejects an invalid or future workspace before replacing data", () => {
    ...
    expect(() =>
      validateImport({ ...createWorkspace(), code: { text: 3 } }),
    ).toThrow();
  });
  it("rejects invalid drawing geometry and mismatched undo snapshots", () => {
    const data = createWorkspace();
    data.board.elements = [
      { id: "bad", type: "not-a-drawing", points: "not-points" },
    ];
  ...
  it("restores an older version only after reviewing the current revision", () => {
    ...
    const edited = editText(
      applyProposal(before, proposal(), "job1"),
      "code",
      "later manual edit",
    );
