...
describe('credential-free runner server', () => {
  it('serves runtime assets locally with restrictive policies', async () => {
    ...
    expect(response.headers.get('content-security-policy')).toContain('frame-ancestors http://localhost:3000');
    expect(response.headers.get('set-cookie')).toBeNull();
    expect(await response.text()).toContain('/bridge.mjs');
    const runtime = await fetch(`${base}/pyodide/pyodide.asm.wasm`, { method: 'HEAD' });
  ...
  it('exposes only an explicit asset allowlist and no application endpoints', async () => {
    for (const path of ['/package.json', '/api/ai', '/%2e%2e/package.json', '/pyodide/package.json', '/server.mjs']) {
      const response = await fetch(`${base}${path}`);
      expect(response.status).toBe(404);
    }
    expect((await fetch(`${base}/`, { method: 'POST', body: 'ignored' })).status).toBe(405);
  });
});
