...
async function run(code: string) {
  ...
  const result = await runtime.execute(
    { id: "test-run", code },
    (channel: string, text: string) => chunks.push({ channel, text }),
  );
  return {
    result,
    chunks,
...
describe("real Pyodide execution", () => {
  ...
  it("keeps actual Python errors and their main.py source line", async () => {
    ...
    expect(result).toMatchObject({
      status: "error",
      line: 2,
      error: expect.stringContaining("ValueError: actual failure"),
    });
  });

  it("completes no-output programs and rejects interactive input", async () => {
    expect((await run("pass")).result.status).toBe("success");
    expect((await run('input("Name? ")')).result).toMatchObject({
      status: "error",
      error: expect.stringContaining("Interactive input is unavailable"),
    });
  });
  ...
  it("caps actual output in bytes and reports overflow", async () => {
    ...
    expect(new TextEncoder().encode(output.stdout).length).toBeLessThanOrEqual(
      65_536,
    );
  ...
  it("restores stdout after a program replaces the sys stream", async () => {
    ...
    expect(
      (
        await isolatedRuntime.execute(
          { id: "first", code: "import sys\nsys.stdout = None" },
          () => {},
        )
      ).status,
    ).toBe("success");
    let output = "";
    const result = await isolatedRuntime.execute(
      { id: "second", code: "print(42)" },
      (_channel: string, text: string) => {
        output += text;
      },
    );
