...
it('serializes writes so a slower old save cannot replace newer work', async () => {
  ...
  const queue = new SaveQueue<number>(async value => { if (value === 1) await blocked; written.push(value); });
...
it('allows a fresh save after a failed write', async () => {
  const values: number[] = [];
  const queue = new SaveQueue<number>(async value => { if (value === 1) throw new Error('storage full'); values.push(value); });
