...
it("does not replace unreadable saved data after editing or retrying save", async () => {
  ...
  const { useWorkspace, hydrateWorkspace, flushSave } =
    await import("../src/features/workspace/store");
  ...
  const { readSavedWorkspace } =
    await import("../src/features/workspace/persistence");
  expect(await readSavedWorkspace()).toEqual({
    schemaVersion: 999,
    valuable: "original work",
  });
});
it("refuses to save content that the next reload would reject", async () => {
  ...
  data.code.text = "x".repeat(200001);
