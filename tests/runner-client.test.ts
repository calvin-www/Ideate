...
class BrowserWindow extends EventTarget { location = { origin: 'http://localhost:3000' }; }
...
function setup() {
  ...
  const emit = (data: Record<string, unknown>, origin = 'http://localhost:3001', source: unknown = frame.contentWindow) => {
...
describe('application runner client', () => {
  it('trusts only the configured frame origin and exact window source', () => {
    ...
    const output = { type: 'output', id: 'run-1', sequence: 0, channel: 'stdout', text: '42\n' };
    ...
    expect(completions).toEqual([['run-1', { status: 'success', durationMs: 1 }]]);
    client.dispose();
  });
  ...
  it('stops immediately and ignores late completion from the stopped run', () => {
    ...
    client.stop();
    expect(frame.sent.at(-1)).toMatchObject({ message: { type: 'stop', id: 'run-1' }, origin: 'http://localhost:3001' });
    emit({ type: 'complete', id: 'run-1', status: 'success', durationMs: 1 });
  ...
  it('marks a run interrupted if its iframe reloads instead of leaving it running', () => {
    ...
    emit({ type: 'status', status: 'ready' });
    expect(completions).toMatchObject([['run-1', { status: 'error', error: expect.stringContaining('reloaded') }]]);
    client.dispose();
  });

  it('validates parent source and allowed origin before accepting any command', () => {
    const parent = {};
    const event = { source: parent, origin: 'http://localhost:3000', data: { ...envelope, type: 'init' } };
    expect(isTrustedParentEvent(event, parent, ['http://localhost:3000'])).toBe(true);
    expect(isTrustedParentEvent({ ...event, source: {} }, parent, ['http://localhost:3000'])).toBe(false);
    expect(isTrustedParentEvent({ ...event, origin: 'https://untrusted.example' }, parent, ['http://localhost:3000'])).toBe(false);
  });
});
