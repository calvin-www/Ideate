...
import type { ArtifactRef, Workspace } from '../../src/features/workspace/model';

type RequestBody = {
  context: { notes: { text: string; length: number; revision: number }; code: { text: string; length: number; revision: number }; board: { revision: number }; sources: ArtifactRef[] };
  continuation?: { contents: unknown[] };
  toolResults?: Array<{ id: string; name: string; result: { status: string } }>;
};
type ReviewPlan = (request: RequestBody) => { text: string; summary: string; target?: 'notes' | 'code' | 'board' };
...
async function fixtureReviews(page: Page, plans: ReviewPlan[]) {
  ...
  await page.route('**/api/ai', async route => {
    ...
    if (body.continuation) {
      ...
      results.push(status);
      events = [{ type: 'text', text: `Review recorded: ${status}.` }, { type: 'done', continuation: { contents: [] } }];
    } else {
      const plan = plans[nextPlan++];
      expect(plan, 'Every AI request has a deterministic fixture').toBeDefined();
      const review = plan(body);
      const target = review.target ?? 'notes';
      const args = target === 'board' ? {
        baseRevision: body.context.board.revision,
        additions: [{ type: 'rectangle', x: 80, y: 80, width: 320, height: 110, text: review.text }],
        updates: [], deleteIds: [], summary: review.summary,
      } : {
        baseRevision: body.context[target].revision,
        replacements: [{ from: target === 'code' ? 0 : body.context[target].length, to: body.context[target].length, text: review.text }],
      ...
      events = [
        ...
        { type: 'call', ...call },
        { type: 'done', continuation: { contents: [{ role: 'model', parts: [{ functionCall: call }] }] } },
      ];
    }
...
function partner(page: Page) { return page.getByRole('complementary', { name: 'AI study partner' }); }
function journal(page: Page) { return page.getByRole('region', { name: 'Study journal', exact: true }); }

async function openJournal(page: Page) {
  await page.goto('/');
  await expect(page.getByText('Saved locally', { exact: true })).toBeVisible({ timeout: 30_000 });
  await page.getByRole('navigation', { name: 'Workspace tools' }).getByRole('button', { name: 'Journal', exact: true }).click();
...
async function askForReview(page: Page, prompt: string) {
  await partner(page).getByRole('textbox', { name: 'Ask your study partner' }).fill(prompt);
  await partner(page).getByRole('button', { name: 'Send message', exact: true }).click();
  await expect(partner(page).getByRole('button', { name: 'Apply', exact: true })).toBeVisible();
}

async function finishReview(page: Page, decision: 'Apply' | 'Reject') {
  await partner(page).getByRole('button', { name: decision, exact: true }).click();
  await expect(partner(page).getByRole('button', { name: 'Stop AI request', exact: true })).toHaveCount(0);
}
...
test('Apply and Reject preserve review control, and an accepted source link opens its excerpt', async ({ page }) => {
  ...
  const fixture = await fixtureReviews(page, [
    ...
    request => {
      linkedSource = request.context.sources.find(source => source.tool === 'code');
  ...
  const sourceLink = journal(page).getByRole('link', { name: 'Original Python source', exact: true });
  ...
  const close = dialog.getByRole('button', { name: 'Close source', exact: true });
...
test('stale Apply and stopping a pending review leave manual notes intact', async ({ page }) => {
  const fixture = await fixtureReviews(page, [
  ...
  const manual = '# My own explanation\n\nI changed my mind while the proposal was open.\n';
  await editNotes(page, manual);
  await finishReview(page, 'Apply');
  await expect(partner(page).getByRole('alert')).toContainText('Your manual edits were preserved');
  ...
  await partner(page).getByRole('button', { name: 'Stop AI request', exact: true }).click();
  await expect(partner(page).getByRole('button', { name: 'Apply', exact: true })).toHaveCount(0);
  await expect(partner(page).getByRole('button', { name: 'Send message', exact: true })).toBeVisible();
...
test('undo conflicts show a review, preserve later edits, and restoration is itself undoable', async ({ page }) => {
  await fixtureReviews(page, [() => ({ text: '\nAn accepted AI observation.\n', summary: 'Record one observation' })]);
  await openJournal(page);
  ...
  await editNotes(page, manual);
  const undo = partner(page).getByRole('button', { name: 'Undo last AI change', exact: true });
  await undo.click();
  await expect(partner(page).getByText('Review restoring an earlier version', { exact: true })).toBeVisible();
  expect((await snapshot(page)).notes.text).toBe(manual);
  await partner(page).getByRole('button', { name: 'Keep current work', exact: true }).click();
  expect((await snapshot(page)).notes.text).toBe(manual);

  await undo.click();
  const latestManual = `${manual}One more correction while reviewing the restore.\n`;
  await editNotes(page, latestManual);
  await partner(page).getByRole('button', { name: 'Restore this version', exact: true }).click();
...
test('Apply & run accepts a fixture code proposal and executes it with the real Python runner', async ({ page }) => {
  const proposedCode = "print('approved execution', 42)\n";
  const fixture = await fixtureReviews(page, [() => ({ target: 'code', text: proposedCode, summary: 'Print the approved execution result' })]);
  await openJournal(page);
  const originalCode = (await snapshot(page)).code.text;
  await askForReview(page, 'Propose Python that prints the approved execution result.');
  expect((await snapshot(page)).code.text).toBe(originalCode);
  await partner(page).getByRole('button', { name: 'Apply & run', exact: true }).click();
  const python = page.getByRole('region', { name: 'Python workspace', exact: true });
  await expect(python).toBeVisible();
  await expect(python.getByLabel('Python output', { exact: true })).toContainText('approved execution 42', { timeout: 60_000 });
  await expect(partner(page).getByRole('button', { name: 'Stop AI request', exact: true })).toHaveCount(0);
  ...
  expect(executed.runs.at(-1)).toMatchObject({ code: proposedCode, revision: executed.code.revision, status: 'success', output: 'approved execution 42\n' });
  expect(fixture.results).toEqual(['accepted']);
});

test('an AI board proposal renders a real preview, applies editable elements, and can be undone', async ({ page }) => {
  const label = 'Keep the sorted half';
  const fixture = await fixtureReviews(page, [() => ({ target: 'board', text: label, summary: 'Add one diagram annotation' })]);
  await openJournal(page);
  await page.getByRole('navigation', { name: 'Workspace tools' }).getByRole('button', { name: 'Whiteboard', exact: true }).click();
  const board = page.getByRole('region', { name: 'Whiteboard tool', exact: true });
  ...
  const preview = partner(page).getByRole('img', { name: 'Preview of the proposed whiteboard changes', exact: true });
  ...
  const rectangle = applied.board.elements.find(element => element.type === 'rectangle' && !element.isDeleted);
  const text = applied.board.elements.find(element => element.type === 'text' && element.text === label && !element.isDeleted);
  ...
  expect(fixture.results).toEqual(['accepted']);
  await partner(page).getByRole('button', { name: 'Undo last AI change', exact: true }).click();
  await expect(page.getByText('Saved locally', { exact: true })).toBeVisible();
  const undone = await snapshot(page);
  expect(undone.board.elements.some(element => element.text === label && !element.isDeleted)).toBe(false);
