...
async function navigate(page: Page, name: 'Desk' | 'Whiteboard' | 'Computer' | 'Journal') {
  await page.getByRole('navigation', { name: 'Workspace tools' }).getByRole('button', { name, exact: true }).click();
}

function pythonPanel(page: Page) { return page.getByRole('region', { name: 'Python workspace', exact: true }); }
function journalPanel(page: Page) { return page.getByRole('region', { name: 'Study journal', exact: true }); }

async function editDocument(page: Page, target: 'code' | 'notes', text: string) {
  const editor = (target === 'code' ? pythonPanel(page) : journalPanel(page)).getByRole('textbox');
  await editor.click();
...
async function runPython(page: Page) {
  const run = pythonPanel(page).getByRole('button', { name: 'Run Python', exact: true });
...
test('binary search executes its real found and missing-target paths', async ({ page }) => {
  ...
  await editDocument(page, 'code', original.code.text.replace('target = 16', 'target = 17'));
  await expect(pythonPanel(page).getByText(/This output is from an earlier version/)).toBeVisible();
  await runPython(page);
  ...
  expect(changed.runs.at(-1)).toMatchObject({ status: 'success', revision: changed.code.revision });
  expect(changed.runs.at(-1)?.code).toContain('target = 17');
});

test('stderr and traceback lines are readable, and empty code succeeds explicitly', async ({ page }) => {
  await openWorkspace(page);
  await navigate(page, 'Computer');
  await editDocument(page, 'code', 'import sys\nprint("before the error", file=sys.stderr)\nraise ValueError("a useful failure")\n');
  await runPython(page);
  const output = pythonPanel(page).getByLabel('Python output', { exact: true });
  ...
  await pythonPanel(page).getByRole('button', { name: 'Go to main.py, line 3', exact: true }).click();
  ...
  await expect(pythonPanel(page).getByRole('button', { name: /Go to main.py, line/ })).toHaveCount(0);
  await editDocument(page, 'code', '');
  await runPython(page);
  await expect(pythonPanel(page).getByText('Completed successfully. This program did not print any output.', { exact: true })).toBeVisible({ timeout: 30_000 });
  expect((await exportedWorkspace(page)).runs.at(-1)).toMatchObject({ status: 'success', code: '', output: '' });
});

test('an infinite loop can be stopped and Python runs again after restarting', async ({ page }) => {
  await openWorkspace(page);
  await navigate(page, 'Computer');
  await editDocument(page, 'code', 'print("loop started")\nwhile True:\n    pass\n');
  await runPython(page);
  await expect(pythonPanel(page).getByText('Python is running', { exact: true })).toBeVisible();
  await pythonPanel(page).getByRole('button', { name: 'Stop', exact: true }).click();
  await expect(pythonPanel(page).getByText('Run stopped. You can edit your code and try again.', { exact: true })).toBeVisible();
  expect((await exportedWorkspace(page)).runs.at(-1)?.status).toBe('cancelled');
  await editDocument(page, 'code', 'print("ready again", 6 * 7)\n');
  await runPython(page);
  await expect(pythonPanel(page).getByLabel('Python output', { exact: true })).toContainText('ready again 42', { timeout: 30_000 });
  expect((await exportedWorkspace(page)).runs.at(-1)?.status).toBe('success');
});

test('code and Markdown notes retain edits across navigation and reload', async ({ page }) => {
  await openWorkspace(page);
  const code = 'prediction = 21 * 2\nprint(prediction)\n';
  const notes = '# A useful invariant\n\nThe array stays sorted.\n\n| Step | Bound |\n| --- | --- |\n| One | 4 |\n\n```python\nlow = mid + 1\n```\n\n<script>window.untrustedMarkdown = true</script>\n';
  await navigate(page, 'Computer');
  await editDocument(page, 'code', code);
  await navigate(page, 'Journal');
  await editDocument(page, 'notes', notes);
  await journalPanel(page).getByRole('tab', { name: 'Preview', exact: true }).click();
  await expect(journalPanel(page).getByRole('heading', { name: 'A useful invariant', exact: true })).toBeVisible();
  ...
  await navigate(page, 'Computer');
  await expect(pythonPanel(page).getByRole('textbox')).toContainText('prediction = 21 * 2');
...
test('the binary-search board example is included in exports and survives reload', async ({ page }) => {
  await openWorkspace(page);
  ...
  await expect.poll(async () => {
    ...
    return saved.board.elements.filter(element => !element.isDeleted).length;
  }, { message: 'The exported workspace contains the loaded board', timeout: 20_000 }).toBeGreaterThan(8);
