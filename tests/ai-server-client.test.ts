...
import {
  createCollaborator,
  type PendingChange,
} from "../src/features/ai/useCollaborator";
import {
  createWorkspace,
  type ArtifactRef,
  type Run,
...
function providerReadsThenNotes(
    ...
    const call = read
      ...
      : step === reads.length
        ? {
            ...
            name: "edit_notes",
            args: {
              baseRevision: notesRevision,
              replacements: [
                {
                  from: 0,
                  to: 0,
                  text: `[Read source](#source:${results.at(-1)!.source.id})\n`,
                },
              ],
...
describe("client AI review lifecycle", () => {
  it("never overwrites a manual edit made while a proposal waits for approval", async () => {
    const provider = providerCall("edit_code", {
      baseRevision: 0,
      replacements: [{ from: 0, to: 0, text: "# proposed\n" }],
  ...
  it("applies an approved patch once and records its source", async () => {
    const provider = providerCall("edit_notes", {
      baseRevision: 0,
      replacements: [{ from: 0, to: 0, text: "An insight.\n" }],
    ...
    expect(data.changes[0].sources[0]).toMatchObject({
      tool: "code",
      revision: 0,
    });
  });
  ...
  it("blocks a notes proposal when its selected Python source changes during review", async () => {
    const provider = providerCall("edit_notes", {
      baseRevision: 0,
      replacements: [{ from: 0, to: 0, text: "Outdated insight.\n" }],
  ...
  it("can capture an immutable saved run even after the live code has changed", async () => {
    ...
    useWorkspace.setState({
      selection: { tool: "code", revision: 0, runId: "saved-run", text: "3\n" },
    });
    const provider = providerCall("edit_notes", {
      baseRevision: 0,
      replacements: [
        { from: 0, to: 0, text: "Run saved-run at revision 0 printed 3.\n" },
      ],
    ...
    expect(useWorkspace.getState().data.changes[0].sources[0].runId).toBe(
      "saved-run",
    );
  });
  ...
  it("stops only the execution owned by its cancelled AI job", async () => {
    ...
    const collaborator = createCollaborator({
      ...
      runCode: () => {
        useWorkspace
          .getState()
          .setData((data) => ({ ...data, runs: [...data.runs, run] }));
      ...
      stopCode: () => {
        stopped++;
        resolveRun?.({ ...run, status: "cancelled" });
      },
    });
  ...
  it("blocks a code proposal when notes included in its initial context change", async () => {
    const provider = providerCall("edit_code", {
      baseRevision: 0,
      replacements: [{ from: 0, to: 0, text: "# based on notes\n" }],
  ...
  it("records a reviewed source link in notes as an undoable artifact change", async () => {
    const provider = providerCall("link_artifacts", {
      sourceIds: ["code"],
      target: "notes",
      summary: "Connect the implementation",
    });
    let pending: PendingChange | null = null;
    const collaborator = createCollaborator({
      ...provider,
    ...
    const asking = collaborator.ask(
      "Link the Python implementation in my notes.",
    );
    await vi.waitFor(() => expect(pending).not.toBeNull());
    await collaborator.approve();
    await asking;
    ...
    const reference = data.references.find((ref) =>
      data.notes.text.includes(`#source:${ref.id}`),
    );
  ...
  it("keeps the initial context immutable when a later read adds a run reference", async () => {
    ...
    expect(
      useWorkspace
        .getState()
        .data.messages.at(-1)
        ?.sources?.some((ref) => ref.runId === "saved-run"),
    ).toBe(true);
  });
  ...
  it.each([
    {
      status: 413,
      message: "This context is too large. Try a smaller selection.",
      streamed: false,
    },
    {
      status: 502,
    ...
    {
      status: 200,
      message: "The answer reached its length limit. Try a smaller step.",
      streamed: true,
    },
  ])(
    "shows the safe endpoint reason for $message",
    async ({ status, message, streamed }) => {
      let shown = "";
  ...
  it.each(["code", "notes"] as const)(
    "saves the exact %s range read beyond the initial source excerpt",
    async (tool) => {
      useWorkspace
        .getState()
        .setText(tool, "x".repeat(3_000) + "return target\nreturn target\n");
      ...
      const asking = collaborator.ask(
        "Read the requested section and link it in my notes.",
      );
      await vi.waitFor(() => expect(pending).not.toBeNull());
      await collaborator.approve();
      await asking;
      expect(provider.results[0].source).toMatchObject({
        tool,
        revision: 1,
        from: 3_000,
        to: 3_014,
        excerpt: "return target\n",
      });
      ...
      expect(data.references.find((ref) => ref.id === source.id)).toEqual(
        source,
      );
      expect(
        data.messages.at(-1)?.sources?.find((ref) => ref.id === source.id),
      ).toEqual(source);
    },
  );
  ...
  it("keeps distinct board subset reads at one revision as distinct saved sources", async () => {
    ...
    const provider = providerReadsThenNotes([
      { name: "read_board", args: { ids: ["left"] } },
      { name: "read_board", args: { ids: ["right"] } },
    ]);
    let pending: PendingChange | null = null;
    const collaborator = createCollaborator({
      request: provider.request,
    ...
    const asking = collaborator.ask(
      "Read both diagram subsets and link the second one.",
    );
    await vi.waitFor(() => expect(pending).not.toBeNull());
    await collaborator.approve();
    await asking;
    expect(provider.results[0].source).toMatchObject({
      ids: ["left"],
      excerpt: "Shared label",
      revision: 1,
    });
    expect(provider.results[1].source).toMatchObject({
      ids: ["right"],
      excerpt: "Shared label",
      revision: 1,
    });
    expect(provider.results[0].source.id).not.toBe(
      provider.results[1].source.id,
    );
    const source = provider.results[1].source;
    const data = useWorkspace.getState().data;
    expect(data.notes.text).toContain(`#source:${source.id}`);
  ...
  it.each([
    ...
    "does not treat an execution mention as authorization: %s",
    async (prompt) => {
      const provider = providerCall("run_python", { revision: 0 });
      let runs = 0;
      const collaborator = createCollaborator({
  ...
  it.each([
    "Run my code",
    "Implement binary search and run it",
    "Can you execute this code?",
  ])("executes a directly requested program: %s", async (prompt) => {
    const provider = providerCall("run_python", { revision: 0 });
    let runs = 0;
    const collaborator = createCollaborator({
      request: provider.request,
    ...
    expect(
      useWorkspace
        .getState()
        .data.messages.at(-1)
        ?.sources?.find((source) => source.runId === "actual-run"),
    ).toMatchObject({ revision: 0, excerpt: "1\n", from: 0, to: 2 });
  });
});
