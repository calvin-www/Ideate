...
const runnerEnv = Object.fromEntries(Object.entries(process.env).filter(([key]) =>
  /^(PATH|SYSTEMROOT|WINDIR|TEMP|TMP|USERPROFILE|LOCALAPPDATA|RUNNER_PORT|RUNNER_APP_ORIGINS)$/i.test(key)));
...
function start(script, args, env) {
  ...
  child.on('exit', (code, signal) => { if (!shuttingDown) shutdown(code ?? (signal ? 1 : 0)); });
  return child;
}
...
start(resolve(root, 'node_modules/next/dist/bin/next'), [production ? 'start' : 'dev', '--hostname', '127.0.0.1'], process.env);
