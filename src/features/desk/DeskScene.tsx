      ...
      makeTexture(1200, 760, (ctx) => {
        if (kind === "code") {
          ...
          ["#b87463", "#c5ab72", "#81a183"].forEach((color, i) => {
            ctx.fillStyle = color;
            ctx.beginPath();
            ctx.arc(1080 + i * 35, 45, 8, 0, Math.PI * 2);
            ctx.fill();
          });
          ...
          lines.forEach((line, i) => {
            ...
            ctx.fillStyle = line.trim().startsWith("#")
              ? "#8fa892"
              : /\b(def|return|while|if|else|elif|for)\b/.test(line)
                ? "#e1c28a"
                : "#d9e5d7";
          ...
          ctx.beginPath();
          ctx.arc(43, 725, 6, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = "#c8d8c6";
          ctx.font = "23px monospace";
          ctx.fillText(
            (status || "Python · ready when you are").slice(0, 70),
            65,
            734,
          );
          return;
        ...
        if (kind === "notes") {
          ...
          const heading =
            content
              .split("\n")
              .find((line) => line.trim())
              ?.replace(/^#+\s*/, "") || "A little clearer, each day.";
    ...
    () =>
      makeTexture(1024, 512, (ctx) => {
        ...
        for (let i = 0; i < 380; i++) {
          const y = i * 1.4;
          ctx.strokeStyle =
            i % 4 === 0 ? "rgba(74,45,24,.075)" : "rgba(233,201,158,.055)";
        ...
        for (const y of [128, 258, 387]) {
          ctx.fillStyle = "rgba(71,41,24,.11)";
...
function Block({
  ...
  return (
    <RoundedBox
      ...
      castShadow
      receiveShadow
      raycast={decorative ? NO_RAYCAST : undefined}
      {...props}
    >
...
function SceneObjects({
  ...
  return (
    <group>
      ...
      <group
        position={[-3.6, 1.42, -1.72]}
        rotation={[0, 0.19, 0]}
        {...hit("board")}
      >
        <Block
          size={[3.45, 2.44, 0.17]}
          color={active === "board" ? "#5d8064" : "#b6b198"}
          radius={0.065}
        />
        <Block
          size={[3.3, 2.28, 0.06]}
        ...
        />
        <Block
          size={[0.62, 0.075, 0.075]}
          position={[0.78, -1.12, 0.19]}
          color="#315d47"
          radius={0.025}
        />
        <Block
          size={[0.19, 0.077, 0.077]}
          position={[1.11, -1.12, 0.19]}
          color="#213c31"
          radius={0.025}
        />
        <Block
          size={[0.54, 0.11, 0.16]}
          position={[-1.0, -1.1, 0.18]}
          color="#d6cfb9"
          radius={0.035}
        />
      </group>
      ...
      <group {...hit("code")}>
        <Block
          size={[1.23, 0.11, 0.85]}
          position={[0.35, -0.028, -1.35]}
          color="#263e34"
          radius={0.05}
        />
        <Block
          size={[0.32, 0.95, 0.18]}
          position={[0.35, 0.43, -1.63]}
          color="#304c3e"
          radius={0.065}
        />
        <group position={[0.35, 1.76, -1.64]} rotation={[-0.075, 0, 0]}>
          <Block
            size={[3.94, 2.57, 0.23]}
            color={active === "code" ? "#6e9276" : "#223b31"}
            radius={0.11}
          />
          <Block
            size={[3.81, 2.43, 0.08]}
            position={[0, 0.02, 0.135]}
            color="#162b24"
            radius={0.07}
          />
          <PreviewPlane
            texture={code}
            size={[3.58, 2.18]}
            position={[0, 0.055, 0.181]}
          />
          <mesh position={[1.6, -1.175, 0.182]}>
            <circleGeometry args={[0.021, 12]} />
            <meshBasicMaterial color="#a4bb8b" />
          </mesh>
        </group>
        <group position={[0.25, 0.023, 0.7]}>
          <Block
            size={[3.0, 0.15, 1.08]}
            color={active === "code" ? "#aab69a" : "#cbcdb9"}
            radius={0.085}
          />
          {Array.from({ length: 4 }, (_, row) =>
            Array.from({ length: 12 }, (_, col) => (
              <Block
                key={`${row}-${col}`}
                size={[0.195, 0.045, 0.16]}
                position={[-1.28 + col * 0.231, 0.095, -0.355 + row * 0.219]}
                color={
                  row === 3 && col > 3 && col < 8
                    ? "#d3d5c2"
                    : col === 11
        ...
        <mesh
          position={[2.14, 0.039, 0.62]}
          scale={[0.24, 0.115, 0.37]}
          castShadow
        >
          <sphereGeometry args={[1, 28, 16]} />
          <meshStandardMaterial color="#d9dbc7" roughness={0.62} />
        </mesh>
      </group>

      <group
        position={[3.58, -0.012, 1.77]}
        rotation={[0, -0.15, 0]}
        {...hit("notes")}
      >
        <Block
          size={[3.34, 0.11, 2.22]}
          color={active === "notes" ? "#78906a" : "#667a54"}
          radius={0.065}
        />
        <Block
          size={[1.6, 0.12, 2.08]}
          position={[-0.805, 0.1, 0]}
          rotation={[0, 0, -0.017]}
          color="#e5dcc0"
          radius={0.035}
        />
        ...
        <mesh position={[0, 0.18, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <planeGeometry args={[3.15, 1.97]} />
          <meshBasicMaterial map={paper} toneMapped={false} />
        </mesh>
        <mesh position={[0, 0.185, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <planeGeometry args={[0.035, 2.0]} />
          <meshStandardMaterial color="#cfc3a4" />
        </mesh>
        <Block
          size={[0.075, 0.01, 0.63]}
          position={[0.4, 0.059, 1.29]}
          color="#b99955"
          radius={0.004}
        />
        <Block
          size={[0.15, 0.14, 1.52]}
          position={[1.21, 0.26, 0.16]}
          rotation={[0, -0.16, 0]}
          color="#b3945a"
          radius={0.035}
        />
  ...
  return (
    <group>
      <group position={[4.7, 0, -2.18]}>
        <mesh position={[0, 0.015, 0]} castShadow raycast={NO_RAYCAST}>
          <cylinderGeometry args={[0.49, 0.54, 0.19, 40]} />
          <meshStandardMaterial color="#546c48" roughness={0.75} />
        </mesh>
        <mesh position={[0, 1.32, 0]} raycast={NO_RAYCAST}>
          <cylinderGeometry args={[0.043, 0.055, 2.58, 16]} />
        ...
        <group position={[-0.27, 2.55, 0.08]} rotation={[0, 0, -0.3]}>
          <mesh castShadow raycast={NO_RAYCAST}>
            <coneGeometry args={[0.67, 0.64, 40, 1, true]} />
            <meshStandardMaterial color="#4b6444" roughness={0.65} side={2} />
          </mesh>
          <mesh
            position={[0, -0.3, 0]}
            ...
            raycast={NO_RAYCAST}
          >
            <circleGeometry args={[0.61, 40]} />
            <meshBasicMaterial color="#f1df9a" side={2} />
          </mesh>
        </group>
      </group>
      ...
      <group position={[-4.42, 0, 1.36]} rotation={[0, 0.1, 0]}>
        ...
        />
        <Block
          size={[1.54, 0.055, 1.97]}
          position={[-0.07, 0.44, -0.03]}
          rotation={[0, -0.09, 0]}
          color="#aa765b"
          radius={0.02}
          decorative
      ...
      <group position={[-2.42, 0.17, 1.45]}>
        <mesh castShadow raycast={NO_RAYCAST}>
          <cylinderGeometry args={[0.29, 0.24, 0.56, 40]} />
          <meshStandardMaterial color="#eee9cf" roughness={0.65} />
        </mesh>
        <mesh
          position={[0, 0.286, 0]}
          rotation={[-Math.PI / 2, 0, 0]}
          raycast={NO_RAYCAST}
        >
          <circleGeometry args={[0.241, 40]} />
          <meshStandardMaterial color="#62482f" roughness={0.45} />
        </mesh>
        <mesh
      ...
      <group position={[1.65, -0.02, -0.45]} rotation={[0, -0.13, 0]}>
        ...
        />
        <Block
          size={[0.47, 0.004, 0.016]}
          position={[-0.025, 0.03, -0.17]}
          color="#a59054"
          radius={0.002}
          decorative
        />
        <Block
          size={[0.36, 0.004, 0.016]}
        ...
        />
        <Block
          size={[0.41, 0.004, 0.016]}
          position={[-0.055, 0.03, 0.13]}
          color="#a59054"
          radius={0.002}
          decorative
        />
...
function CameraMotion({
  ...
  transition: { tool: Tool; id: number } | null;
  onComplete: (tool: Tool) => void;
}) {
  ...
  useEffect(() => {
    ...
    motion.current = {
      ...
      endPosition: camera.position
        .clone()
        .lerp(target.clone().add(new Vector3(3.1, 3.2, 5.0)), 0.25),
            ...
            <span className={styles.fallbackPreview} aria-hidden="true">
              {tool === "board" ? (
                ...
                <>
                  ...
                  <code>{codePreview.split("\n").slice(0, 6).join("\n")}</code>
                  <span className={styles.fallbackStatus}>
...
export default function DeskScene(props: DeskSceneProps) {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) =>
      setSmallScreen(entries[0].contentRect.width < 680),
    );
    if (root.current) observer.observe(root.current);
    return () => {
      observer.disconnect();
      media.removeEventListener("change", updateMotion);
      cleanContextListener.current?.();
    };
  }, []);
  ...
  return (
    <div
      ...
      data-active-object={active || undefined}
      aria-label="Your study desk"
    >
      {smallScreen || contextLost ? (
        fallback
      ) : (
