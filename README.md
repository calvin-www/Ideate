...
Use the logo's **Back to desk** link to choose a tool. Whiteboard, Computer, and Journal always open alone from the desk, preserving their content and editor state. Use **Layout** to add a tool beside or below the focused pane; tabs and floating editors are available under **More arrangements**. Drag tab headers to rearrange editors, floating title bars to move them, and dividers or window edges to resize them. Group controls let you dock, maximize, or hide an editor. **Restore arrangement** (or Escape while maximized) exits maximization. **Show only [tool]** hides other panes; **Restore previous arrangement** explicitly recovers the most recent arrangement, including after desk navigation or reload. There is one shared arrangement, stored separately from documents in this browser. Editor undo history and running Python survive layout changes. Computer opens with output embedded. Narrow windows temporarily show one editor without overwriting the desktop arrangement; selecting a desk object still opens only that tool. Separate browser popout windows are not included.
